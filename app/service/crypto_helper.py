"""Local crypto primitives ported from the fully-local `me-cli-sunset` fork.

This replaces dead remote `me-crypto.mashu.lol` calls. All the real work that the
operator's `/api/890/*` endpoints used to do happens here in-process with pycryptodome.
The env keys referenced here are read at import; two of them (XDATA_KEY,
X_API_BASE_SECRET) are genuinely new and must be copied from the phone's .env.
"""
...
def make_x_signature_balance_allotment(
    ...
    # DOR-XL-specific (no verified reference). Mirrors the sibling
    # make_x_signature_bounty_allotment layout; if XL rejects sharing balance
    # allotment, this key/msg layout is the place to re-derive.
    key_str = f"{X_API_BASE_SECRET};{sig_time_sec}#ae-hei_9Tee6he+Ik3Gais5=;{msisdn};POST;{path};{sig_time_sec}"
    key_bytes = key_str.encode("utf-8")

    msg = f"{access_token};{sig_time_sec};{msisdn};{amount};".encode("utf-8")
